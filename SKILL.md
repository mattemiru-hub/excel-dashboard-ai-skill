...
When possible, group the intake questions into one short block so the user can answer in one message.
...
- Short intake summary stating which raw sheet, objective, date field, actual metric, target field, slicers, and rebuild/polish mode were confirmed
